...
function PrayTime() {
	...
	*/
    ...
    this.methodParams[this.MWL] = new Array(20, 1, 0, 0, 18);
...
PrayTime.prototype.getDatePrayerTimes = function (
    year,
    month,
    day,
    latitude,
...
// return prayer times for a given date
PrayTime.prototype.getPrayerTimes = function (
    date,
    latitude,
    longitude,
    timeZone
...
PrayTime.prototype.setCustomParams = function (params) {
    for (var i = 0; i < 5; i++) {
        if (params[i] == null)
            this.methodParams[this.Custom][i] =
                this.methodParams[this.calcMethod][i];
        else this.methodParams[this.Custom][i] = params[i];
    }
...
PrayTime.prototype.floatToTime12 = function (time, noSuffix) {
    ...
    return (
        hours + ":" + this.twoDigitsFormat(minutes) + (noSuffix ? "" : suffix)
    );
};
...
PrayTime.prototype.sunPosition = function (jd) {
    ...
    var L = this.fixangle(q + 1.915 * this.dsin(g) + 0.02 * this.dsin(2 * g));
...
PrayTime.prototype.computeAsr = function (
    step,
    t // Shafii: step=1, Hanafi: step=2
) {
    var D = this.sunDeclination(this.JDate + t);
...
PrayTime.prototype.computeTimes = function (times) {
    ...
    var Fajr = this.computeTime(
        180 - this.methodParams[this.calcMethod][0],
        t[0]
    );
...
PrayTime.prototype.adjustTimes = function (times) {
    for (var i = 0; i < 7; i++) times[i] += this.timeZone - this.lng / 15;
...
PrayTime.prototype.adjustHighLatTimes = function (times) {
    ...
    // Adjust Fajr
    var FajrDiff =
        this.nightPortion(this.methodParams[this.calcMethod][0]) * nightTime;
    if (isNaN(times[0]) || this.timeDiff(times[0], times[1]) > FajrDiff)
        times[0] = times[1] - FajrDiff;

    // Adjust Isha
    var IshaAngle =
        this.methodParams[this.calcMethod][3] == 0
    ...
    // Adjust Maghrib
    var MaghribAngle =
        this.methodParams[this.calcMethod][1] == 0
